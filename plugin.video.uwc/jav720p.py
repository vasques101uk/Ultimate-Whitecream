...
def Main():
    utils.addDir('[COLOR hotpink]Categories[/COLOR]','http://jav720p.com/',323,'','')
    utils.addDir('[COLOR hotpink]Search[/COLOR]','http://jav720p.com/?s=',324,'','')
    List('http://jav720p.com/page/1/?orderby=date')
...
def Categories(url):
    ...
    match = re.compile('<a href="(http://jav720p.com/category/[^"]+)" >([^<]+)<', re.DOTALL | re.IGNORECASE).findall(cathtml)
